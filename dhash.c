...
void insert_in_hash(HashTable *dict, char *str)
{
   ...
   find_space(dict->array, hash, str, dict); /* Finds empty cell in hash table and inserts word */
...
Word **bigger_array(HashTable *dict, int newlength)
{
   ...
   newtable = (Word **)malloc(newlength*sizeof(Word));
   initialise(newtable, newlength);
   ...
   for (i=0;i<dict->size;i++){
      if (dict->array[i]->string!=NULL){ /* Only hashes strings if they have been filled */
         ...
         find_space(newtable, hash, dict->array[i]->string, dict); /* Inserts word into appropriate cell */
      }
      free(dict->array[i]); 
   }
   free(dict->array);
